...
class Manipulacao{
    ...
    public function setGw($gw){$this->gw = $gw;}
    ...
    public function listar($int, $table){
        ...
        if($int == null){
            $requisicao = $this->banco->query("SELECT * FROM $table WHERE classificacao = 'externa' ORDER BY ethernet");
        }else{
    ...
    public function adicionar(){
        ...
        try{
            $this->banco->exec("INSERT INTO interfaces VALUES ('$this->interface', '$this->tipo_placa', '$this->modo', '$this->class_placa', '$this->ip', '$this->mask', '$this->gw', '$this->rede', '$this->bcast', '$this->dns1', '$this->dns2', '$this->mac', $this->status)");
        }catch(Exception $msg){
            die( $msg->getMessage() );
            return $msg;
        }
    ...
    public function editar(){
        ...
        $this->banco->enableExceptions(true);
        try{
            $this->banco->exec("UPDATE interfaces SET ethernet='$this->interface', tipo_placa = '$this->tipo_placa',  modo = '$this->modo', classificacao = '$this->class_placa', ip='$this->ip', mascara='$this->mask', gateway = '$this->gw', rede='$this->rede',broadcast='$this->bcast', dns1 = '$this->dns1', dns2 = '$this->dns2', mac = '$this->mac', status = $this->status WHERE ethernet = '$this->interface'");
        }catch(Exception $msg){
            die( $msg->getMessage() );
            return $msg;
        }
